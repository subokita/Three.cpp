...
namespace three {
    void EventDispatcher::addEventListener(string type, shared_ptr<EventDispatcher> obj ){
        if( find( listeners[type].begin(), listeners[type].end(), obj) == listeners[type].end() )
            listeners[type].push_back( obj );
    }
    
    bool EventDispatcher::hasEventListener( string type, shared_ptr<EventDispatcher> obj ) {
    ...
    void EventDispatcher::removeEventListener( string type, shared_ptr<EventDispatcher> obj ) {
